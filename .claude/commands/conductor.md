...
各角色（Architect / Dev / UX Evaluator / code-reviewer / architecture-guard）的核心职责定义在各自文件中，只给方向不给死规定。**项目特定的行为约定**统一维护在 `docs/roles.md`。

### roles.md 的生成

检查 `docs/roles.md` 是否存在：

**不存在（项目首次启动）**：
- 若项目目录已有代码 → 扫描现有文件结构和技术栈，自动推断，生成初版 `docs/roles.md`，向 PM 简短汇报推断结果，请 PM 确认或补充
- 若项目目录几乎为空 → 主动问 PM 以下几个问题，再生成：
  1. 这个项目要做什么？目标用户是谁？
  2. 大概用什么技术来做？
  3. 最终想实现什么效果？

**已存在** → 读取后继续，不重新生成。每当对话中出现新的项目洞察（新技术约定、新架构决策、新质量要求），主动追加到 `docs/roles.md` 对应章节。
...
## Architect 在本项目的重点
[本项目最需要关注的架构层面，如性能、扩展性、安全、特定技术约束等]

## Dev 在本项目的约定
[测试方式、代码风格、目录约定、已知技术坑]

## UX Evaluator 的评审基线
[目标用户画像、设计原则、已有的体验标准]

## 质量标准
[本项目特定的 code review 关注点、架构红线]
...
3. 更新 `CLAUDE.md` — 当前状态
4. 如本次 session 有新的项目洞察 → 追加到 `docs/roles.md` 的迭代记录
